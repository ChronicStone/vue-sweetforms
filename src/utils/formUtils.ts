...
export const MapFormInitialState = (fields: any[], inputFormData: any = {}, parentKey = "") => {
    ...
    fields.forEach((field: any) => {
        console.log({ fieldKey: field.key, fieldVal: inputFormData[field.key]})
        if(!['array', 'object'].includes(field.type)) state[field.key] = inputFormData?.[field.key] ? inputFormData?.[field.key] : field?.default ? field?.default : field.type === 'checkbox' ? false : field.type === 'number' ? 0 : null
