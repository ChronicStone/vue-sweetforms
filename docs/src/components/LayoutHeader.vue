  ...
  <div class="py-2 border-t-2 border-ui-primary">
    <div class="container">
      <div class="flex items-center justify-between -mx-2 sm:-mx-4">
        <div
          class="flex flex-col items-center px-2 mr-auto sm:px-4 sm:flex-row gap-4"
        >
          <g-link to="/" class="flex items-center text-ui-primary pr-2" title="Home">
            <Logo :width="40" />
            <span
          ...
          <div
            ...
            class="hidden mr-5 sm:flex sm:ml-8 gap-3 ml-2"
          >
            <g-link
              v-for="link in settings.nav.links"
              :key="link.path"
              :to="link.path"
              class="p-1 px-2 font-medium transition-colors duration-300 ease-out border-b-2 nav-link border-ui-background hover:text-ui-primary"
            >
              {{ link.title }}
            </g-link>
          </div>
...
const Search = () =>
  import(/* webpackChunkName: "search" */ '@/components/Search').catch(err =>
