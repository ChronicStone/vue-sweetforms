        ...
        >
          <g-link
            ...
            :class="{
              ...
              'font-bold text-ui-primary translate-x-4': isCurrentHeading(
                heading
              ),
              'hover:translate-x-1': !isCurrentHeading(heading)
            }"
          >
            <span
              class="absolute w-2 h-2 -ml-4 transition origin-center transform scale-0 rounded-full opacity-0 bg-ui-primary"
              :class="{
                'opacity-100 scale-100': isCurrentHeading(heading)
              }"
            ></span>
            {{ heading.value }}
          </g-link>
...
export default {
  ...
  watch: {
    $route: function () {
      this.createNextObserver(() => this.observer.disconnect())
    }
  },

  mounted() {
    this.createNextObserver()
  },

  methods: {
    isCurrentHeading({ anchor }) {
      return this.activeAnchor === anchor
    },
    ...
    observerCallback(entries) {
      ...
      // We want to give the link of the intersecting headline active
      if (id) {
